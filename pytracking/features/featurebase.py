...
class MultiFeatureBase(FeatureBase):
    ...
    def size(self, im_sz):
        if self.output_size is None:
            ...
            return TensorList([torch.div(im_sz, s, rounding_mode='trunc') for s in self.stride()])
        if isinstance(im_sz, torch.Tensor):
            #return TensorList([im_sz // s if sz is None else torch.Tensor([sz[0], sz[1]]) for sz, s in zip(self.output_size, self.stride())])
            return TensorList([torch.div(im_sz, s, rounding_mode='trunc') if sz is None else torch.Tensor([sz[0], sz[1]]) for sz, s in zip(self.output_size, self.stride())])
